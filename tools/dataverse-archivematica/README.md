...
| `verifica_raggiungibilita.py` | *Migrazione (sola lettura).* Data una lista di DOI, verifica che ognuno risponda ancora su Dataverse, ne rileva stato (pubblicato/bozza/accesso negato/ristretto) e stima il volume di ri-acquisizione con gli originali. Produce un report e l'elenco dei DOI non raggiungibili. Resumibile; non cancella nulla |
...
- **Conservazione dell'originale per i file tabellari**: i file ingeriti da
  Dataverse (Stata, SPSS, CSV, xlsx…) vengono serviti di default come derivata
  `.tab`, generata al volo e con checksum instabile. La pipeline scarica invece
  l'**originale** depositato (`format=original`), salvandolo col nome originale
  (`.csv`, `.dta`…): è l'oggetto autentico da conservare secondo OAIS, ed è
  Archivematica — non Dataverse — a produrre le derivate di preservazione. Se
  `format=original` non è disponibile (404), la pipeline ripiega sulla `.tab`
  in modalità degradata tracciata a log (`[FALLBACK]`)
- **Verifica di integrità dei file scaricati**: `scarica_dataverse.py` confronta
  ...
  attesa). Per i file ingeriti il checksum del manifest si riferisce all'originale
  (verificato empiricamente su dataverse.unimi.it), quindi la verifica sull'originale
  scaricato è **fixity forte**. Un file già presente viene saltato solo se supera il
  controllo; un download troncato o corrotto viene rimosso e ri-scaricato invece di
